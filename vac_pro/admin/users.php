...
require_hr_admin();
$pdo=db();
$msg='';$err='';$temporaryPassword='';

function admin_temp_password(): string
{
    $upper='ABCDEFGHJKLMNPQRSTUVWXYZ';$lower='abcdefghijkmnopqrstuvwxyz';$digits='23456789';$all=$upper.$lower.$digits;
    $pw=$upper[random_int(0,strlen($upper)-1)].$lower[random_int(0,strlen($lower)-1)].$digits[random_int(0,strlen($digits)-1)];
    while(strlen($pw)<10)$pw.=$all[random_int(0,strlen($all)-1)];
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        ...
        if($action==='reset_password'){
            if(!is_super_admin())throw new RuntimeException('비밀번호 초기화는 최고관리자만 가능합니다.');
            $targetId=(int)($_POST['target_id']??0);
            $q=$pdo->prepare('SELECT id,username,name FROM admins WHERE id=?');$q->execute([$targetId]);$target=$q->fetch();
            if(!$target)throw new RuntimeException('초기화할 계정을 찾지 못했습니다.');
            $temporaryPassword=admin_temp_password();
            $pdo->prepare('UPDATE admins SET password_hash=? WHERE id=?')->execute([password_hash($temporaryPassword,PASSWORD_DEFAULT),$targetId]);
            $msg=$target['name'].' 계정의 임시 비밀번호가 발급되었습니다.';
        }else{
